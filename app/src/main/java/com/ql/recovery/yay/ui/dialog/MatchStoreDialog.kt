...
class MatchStoreDialog(
    ...
    private fun initVew() {
        binding = DialogMatchStoreBinding.inflate(layoutInflater)
        ...
        binding.tvGetMore.setOnClickListener { loadMoreServerList() }
            ...
            .addBindView { itemView, itemData ->
                ...
                when (itemData.code) {
                    "coin_level_one", "" -> {
                        ...
                        itemBinding.root.background = ResourcesCompat.getDrawable(activity.resources, R.drawable.hybg_q, null)
                        itemBinding.llMoney.background = ResourcesCompat.getDrawable(activity.resources, R.drawable.hybg_m, null)
                        itemBinding.tvMoney.setTextColor(Color.BLACK)
                        itemBinding.tvCount.setTextColor(Color.BLACK)
                        itemBinding.ivOriginPrice.visibility = View.GONE
                    ...
                    "coin_level_two" -> {
                        ...
                        itemBinding.llMoney.background = ResourcesCompat.getDrawable(activity.resources, R.drawable.hybg_b, null)
                        ...
                        itemBinding.tvMoney.setTextColor(Color.WHITE)
                        itemBinding.ivOriginPrice.visibility = View.GONE
                    ...
                    "member_subscribe_monthly" -> {
                        ...
                        itemBinding.ivOriginPrice.visibility = View.VISIBLE
                    }
    ...
    private fun loadServerList() {
        DataManager.getProductList("lemon") { list ->
            ...
            mList.clear()
            for (child in list) {
                if (child.code == "coin_level_two" || child.code == "member_subscribe_monthly") {
                    mList.add(child)
                    mAdapter.notifyItemInserted(mList.size - 1)
                }
            }

//            PayManager.get().getSubProductList(activity, billingClient, mList) { position ->
//                mAdapter.notifyItemChanged(position)
//            }
        }
    }

    private fun loadMoreServerList() {
        binding.tvGetMore.visibility = View.GONE
        val height = AppUtil.getScreenHeight(activity)
        val lp = binding.layoutParent.layoutParams
        lp.height = height * 3 / 4
        binding.layoutParent.layoutParams = lp
