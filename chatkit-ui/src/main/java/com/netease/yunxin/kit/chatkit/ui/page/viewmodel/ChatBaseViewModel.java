...
public abstract class ChatBaseViewModel extends BaseViewModel {
     ...
     * 免费用户增加每日使用次数
     */
    private void addUsageTime() {
        com.ql.recovery.bean.UserInfo userInfo = mk.decodeParcelable("user_info", com.ql.recovery.bean.UserInfo.class);
        if (userInfo != null && !userInfo.is_vip() && !userInfo.getRole().equals("anchor")) {
            int times = mk.decodeInt("daily_msg_time", 0);
            times = times + 1;
            mk.encode("daily_msg_time", times);
