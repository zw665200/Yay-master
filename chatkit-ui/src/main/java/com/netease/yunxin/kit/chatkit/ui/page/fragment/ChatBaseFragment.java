...
public abstract class ChatBaseFragment extends BaseFragment {
    ...
    protected void initView() {
        ...
        binding.chatView.getTitleBar().getRightImageView().setOnClickListener(v -> {
            ...
            //弹出菜单
            new MenuDialog(requireActivity(), Integer.parseInt(viewModel.getSessionId()), isBlock, block -> {
                if (block) {
                    //移除黑名单
                    NIMClient.getService(FriendService.class).removeFromBlackList(viewModel.getSessionId())
                            .setCallback(new RequestCallback<Void>() {
                                @Override
                                public void onSuccess(Void result) {
                                    ToastUtil.showShort(requireContext(), "remove success");
                                }

                                @Override
                                public void onFailed(int code) {
                                    ToastUtil.showShort(requireContext(), "remove failed");
                                }
                    ...
                    //加入黑名单
                    NIMClient.getService(FriendService.class).addToBlackList(viewModel.getSessionId())
                            .setCallback(new RequestCallback<Void>() {
                                @Override
                                public void onSuccess(Void result) {
                                    ToastUtil.showShort(requireContext(), "add success");
                                }

                                @Override
                                public void onFailed(int code) {
                                    ToastUtil.showShort(requireContext(), "add failed");
                                }
        ...
        binding.chatView.getTitleBar().getRightImageView().setImageResource(R.drawable.lt_dian);
