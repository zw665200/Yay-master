...
class ModifyDialog(
    ...
    private fun initVew() {
        ...
        binding.tvModifyTitle.text = title
        binding.etModifyContent.setText(userInfo.nickname)
    ...
    private fun commit() {
        val name = binding.etModifyContent.editableText.toString()
        if (name.isBlank() || name == title) {
            return
        }

        val nil = null
        DataManager.updateUserInfo(nil, nil, name.trim(), nil, nil, nil, nil) {
            if (it) {
                cancel()
                func(name)
            }
