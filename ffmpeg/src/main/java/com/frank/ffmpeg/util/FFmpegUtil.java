...
//        String transformVideoCmd = "ffmpeg -i %s -r 25 -b 200 -s 1080x720 %s";
    ...
    public static String[] cutVideoWithoutAudio(String inputPath, float startTime, float duration, String outputPath) {
        // -map 0 -codec copy (copy all tracks)
        // -map 0:v -vcodec copy (copy track of video)
        // -map 0:a -acodec copy (copy all tracks of audio)
        // -map 0:s -scodec copy (copy all tracks of subtitle)
//        String cutVideoCmd = "ffmpeg -ss %f -t %f -accurate_seek -i %s -y -vcodec copy -acodec copy -avoid_negative_ts 1 -an %s";
        String cutVideoCmd = "ffmpeg -ss %f -t %f -i %s -threads 2 -y -c:v copy -c:a copy %s";
        cutVideoCmd = String.format(Locale.getDefault(), cutVideoCmd, startTime, duration, inputPath, outputPath);
        return cutVideoCmd.split(" ");
    }
     ...
     * @return
     */
    public static String[] videoToGif(String inputPath, int startTime, int duration, String outputPath) {
        String cmd = "ffmpeg -y -i %s -ss %d -t %d -r 15 %s";
        cmd = String.format(Locale.getDefault(), cmd, inputPath, startTime, duration, outputPath);
        return cmd.split(" ");
    }
